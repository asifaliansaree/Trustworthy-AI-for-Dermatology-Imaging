...
def compute_saliency(model, tensor, target_class, device):
    model.eval()
    sal  = Saliency(model)
    attr = sal.attribute(tensor.to(device), target=target_class)
    hm   = attr.squeeze().cpu().detach().numpy()
    hm   = np.mean(np.abs(hm), axis=0)
    return normalize_attr(hm)

def run_saliency_batch(cases, out_dir, model, device, max_cases=20):
    ...
    for i, case in enumerate(cases[:max_cases]):
        ...
        heatmap = compute_saliency(model, tensor, pred, device)
        overlay = overlay_on_image(display, heatmap)
        save_triple_figure(
            display, heatmap, overlay,
            title=f"Saliency | true={case['true_label']} pred={CLASSES[pred]} conf={conf:.3f}",
            pred_name=CLASSES[pred], true_name=case['true_label'],
            conf=conf,
            save_path=os.path.join(out_dir, f"{i:02d}_{case['image_id']}_sal.png"),
            method_name="Vanilla Saliency",
        )
        print(f"  [{i+1:02d}] {case['image_id']}: done")
